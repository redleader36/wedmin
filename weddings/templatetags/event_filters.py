...
#     # url = "%s/events/%s" % (current_site.domain, event.id)
#     name = urllib.parse.quote_plus(event.name)
#     iso_begin = event.iso_begin
#     iso_end = event.iso_end
#     description = urllib.parse.quote_plus(event.description)
#     venue = "%s, %s" % (event.venue, event.address)
#     address = urllib.parse.quote_plus(venue)
#     link = "http://www.google.com/calendar/event?action=TEMPLATE&text=%s&dates=%s//%s&details=%s&location=%s&sprop=website:%s&trp=false" % (name, iso_begin, iso_end, description, address, url)
#     return link
